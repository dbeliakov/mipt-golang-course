...
Реализуйте функцию `RLECompress`, которая производит RLE-сжатие строки: на вход подается строка, состоящая из часто повторяющихся символов латинского алфавита (a-z, A-Z). Функция должна вернуть строку, в которой все идущие подряд одинаковые символы заменены на количество повторений и один экземпляр символа:
`aabbc -> 2a2b1c`.
...
BenchmarkRLECompress-12           261058              4548 ns/op            5368 B/op         10 allocs/op
PASS
```
