...
- **Комментарии:** если строка после удаления пробелов по краям начинается с `;` или `#`, вся строка — комментарий и игнорируется. Иначе из строки удаляется *инлайн-комментарий*: первый символ `;` или `#`, находящийся **вне** двойных кавычек `"`, и всё до конца строки.
...
- Имена заголовков должны иметь ненулевую длину (после `TrimSpace` внутри скобок).
- Первый символ `=` **вне двойных кавычек** разделяет ключ и значение. До и после `=` может находиться произвольное количество пробельных символов.
- Значение может быть в двойных кавычках: `key = "..."`. Внутри кавычек допускается символ `=` и символы `#`, `;`. После закрывающей кавычки не должно оставаться непробельных символов.
- Если значение **не** в кавычках, оно не может содержать символ `=` и не может содержать кавычки `"`.
