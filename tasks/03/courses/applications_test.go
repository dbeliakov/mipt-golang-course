...
func TestProcessApplications(t *testing.T) {
	testCases := []struct {
		...
		expectedUnassigned []StudentApplication
	}{
		{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			byCourse, unassigned := ProcessApplications(tc.students, tc.courses)
			assert.Equal(t, tc.expected, byCourse)
			assert.Equal(t, tc.expectedUnassigned, unassigned)
		})
	}
}
