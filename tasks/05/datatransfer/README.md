...
**Корректное завершение (graceful shutdown):** при любой фатальной ошибке (не `ErrProducerFinished`) или при отмене контекста нужно **остановить остальные горутины согласованно**: используйте производный `context.Context` с отменой и вызывайте `cancel()`, когда трансфер должен прекратиться (ошибка продюсера, ошибка записи, отмена). Это предотвращает зависания и лишнюю работу после первой ошибки.

Проверка на истечение контекста происходит с помощью чтения из канала `ctx.Done()`. 
Самое частое использование — ожидать одновременно двух или более блокирующих событий, одним из которых является отмена контекста:
