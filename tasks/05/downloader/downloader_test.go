...
// peakTrackingTransport считает максимальное число одновременных RoundTrip.
type peakTrackingTransport struct {
	inner    http.RoundTripper
	inFlight atomic.Int32
	peak     atomic.Int32
}

func (p *peakTrackingTransport) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	for {
		...
		if p.peak.CompareAndSwap(old, cur) {
			break
		}
	}
...
func TestDownloader_Download_ConcurrencyLimit(t *testing.T) {
	const maxConcurrent = 3
	const numURLs = 15

	responses := make(map[string]*http.Response)
	for i := 0; i < numURLs; i++ {
		u := fmt.Sprintf("http://concurrent.test/%d", i)
		responses[u] = &http.Response{
			StatusCode: http.StatusOK,
			Body:       io.NopCloser(bytes.NewBufferString("ok")),
		}
	}

	base := &mockTransport{responses: responses}
	tracker := &peakTrackingTransport{inner: base}

	client := &http.Client{Transport: tracker}
	d := NewDownloaderWithConcurrency(client, maxConcurrent)

	urls := make([]string, 0, numURLs)
	for i := 0; i < numURLs; i++ {
		urls = append(urls, fmt.Sprintf("http://concurrent.test/%d", i))
	}

	results, errs := d.Download(urls...)
	require.Empty(t, errs)
	require.Len(t, results, numURLs)

	assert.LessOrEqual(t, int(tracker.peak.Load()), maxConcurrent,
		"число одновременных HTTP-запросов не должно превышать лимит пула")
}
