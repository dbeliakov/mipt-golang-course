...
**Ограничение параллелизма:** используйте пул из фиксированного числа рабочих горутин (`worker pool`): одновременно не должно выполняться больше `N` HTTP-запросов, где `N` задаётся при создании загрузчика. Если лимит не задан (`0`), допустимо вести себя как раньше — запускать по горутине на каждый URL.

Конструкторы:
- `NewDownloader(client *http.Client)` — без лимита параллелизма;
- `NewDownloaderWithConcurrency(client *http.Client, maxConcurrent int)` — не более `maxConcurrent` одновременных скачиваний (если `maxConcurrent` больше числа URL, фактически работает `len(urls)` горутин).
...
- тело запроса `Response.Body` обязательно нужно закрывать, иначе соединение не вернется в пул и данные не освободятся
